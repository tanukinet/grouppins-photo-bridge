...
2. 初回はここで権限 (メディアの位置情報 + 写真へのアクセス) を要求する。写真へのアクセスは
   **「すべて許可」が必要**で、Android 14 以降の「写真を選択」(一部のみ) は権限不足として扱い、
   ...
   変更手順と「設定を開く」ボタンを載せた警告ダイアログを出し、どのボタンを押しても終了する。
   「設定を開く」はこのアプリのアプリ情報画面へ直接飛ぶ。手順は下の
...
- 権限結果の判定は `requiredPermissions()` の全件が付与されたかで行う (`grantResults` の
  ...
  トーストでは読み切る前に消えるため。中立ボタンの「設定を開く」は `PhotoBridge.openAppSettings`
  を呼ぶ。完全拒否の経路が使っている `ACTION_APPLICATION_DETAILS_SETTINGS` と同じ行き先なので、
  intent の組み立ては 1 箇所にまとめてある。どのボタンでも Back でも外側タップでも `finish()`
  して、透明な画面が残らないようにする。閉じる処理は `dismissNotice` に集約し、`onNewIntent`
